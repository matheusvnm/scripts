...
urano_metrics=("performance" "temperature")
budget_power_values=("25" "50" "75" "100")
...
for config in "${configs[@]}"; do
    ...
    elif [ $config = "omp_budget" ]; then
        compile_technique
        for mode in "${budget_power_modes[@]}"; do
            set_turbo $mode $config
            for power_budget in "${budget_power_values[@]}"; do
    ...
    elif [ $config = "omp_dynamic" ]; then
        for mode in "${omp_dynamic_modes[@]}"; do
